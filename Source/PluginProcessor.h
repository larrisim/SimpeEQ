...
struct Fifo
{
    void prepare(int numChannels, int numSamples)
    {
        static_assert( std::is_same_v<T, juce::AudioBuffer<float>>,
                      "prepare(numChannels, numSamples) should only be used when the Fifo i holding juce::AudioBuffer<float>");
    ...
    void prepare(size_t numElements)
    {
        static_assert( std::is_same_v<T, std::vector<float>>,
                      "prepare(numElements) should only be used when the Fifo i holding std::vector<float>");
...
Coefficients makePeakFilter(const ChainSettings& chaninSettings, double sampleRate);
